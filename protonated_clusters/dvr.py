...
hyd_shifts = np.linspace(1.95725269-1, 1.95725269 + 1.25, 10)  # bohr
...
def get_geometries(eq_h7o3_load, ox_shifts_load, hyd_shifts_load, ox_1, ox_2, hyd_1): #ox_1 = (0,0,0) ox_2 = 4, hyd_1 = 3
    # geoms = np.zeros(np.shape(eq_h7o3_load))
    # print(geoms)
    # geoms = np.empty((10, 3), dtype=int)
    eq_h7o3_load[hyd_1, 1:] = 0  # y & z = 0
    eq_h7o3_load[ox_2, 0] -= (4 * .25)  # shift ox back
    ct = 1
    for i in ox_shifts_load:
        eq_h7o3_load[ox_2, 0] += i
        for j in hyd_shifts_load:
            # geoms = np.zeros(np.shape(eq_h7o3_load))
            print(np.shape(eq_h7o3_load))
            eq_h7o3_load[hyd_1, 0] += j
            geoms[:] = eq_h7o3
